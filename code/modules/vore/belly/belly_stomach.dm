			...
			if (iscarbon(M) || isanimal(M)) // If human or simple mob and you're set to digest.
				...
				if(air_master.current_cycle%3==1)
					if(!(M.status_flags & GODMODE))
						var/noiseChance = prob(10)
						if (noiseChance == 1)
							var/noisePick = prob(12) // There's a more efficient way of doing this but right now I'm lazy and not looking it up. -Ace
