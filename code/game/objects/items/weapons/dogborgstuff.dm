...
/obj/item/device/dogborg/sleeper/proc/go_out()
	if(src.occupied == 0)
		return
	loc << "<span class='notice'>[patient] ejected. Life support functions disabled.</span>"
...
/obj/item/device/dogborg/sleeper/proc/clean_cycle()
	...
	if(length(contents) == 0)
		R << "<span class='notice'>Your sleeper is now clean. Ending self-cleaning cycle.</span>"
		cleaning = 0
		update_patient()
		return
	...
	if(air_master.current_cycle%3==1) //This is how the vore code times it, so I'm using that
		...
		R << "DEBUG: Now targeting [target.name]"
		...
		if(istype(target, /mob/living/carbon/human))
			...
			if((T.status_flags & GODMODE) || !T.digestable)
				...
				R.visible_message("<span class='notice'>[T.name] spills out of [R.name]'s [eject_port].</span>", "<span class='notice'>[T.name] spills out of your [eject_port].</span>")

			//Mob is now dead
			if(T.stat & DEAD)
				R << "DEBUG: [T.name] is dead, cleaning up."
				message_admins("[key_name(R)] has digested [key_name(T)] as a dogborg. ([R ? "<a href='?_src_=holder;adminplayerobservecoodjump=1;X=[R.x];Y=[R.y];Z=[R.z]'>JMP</a>" : "null"])")
				R << "<span class='notice'>You feel your belly slowly churn around [T], breaking them down into a soft slurry to be used as power for your systems.</span>"
				T << "<span class='notice'>You feel [R]'s belly slowly churn around your form, breaking you down into a soft slurry to be used as power for [R]'s systems.</span>"
				drain(-30000) //Fueeeeellll
				for(var/obj/SubItem in T.contents) //Taking mob's items and putting them in belly
					SubItem.loc = src
					if(istype(SubItem,/mob/living)) //Friendly message for now-exposed mobs c:
						SubItem << "As your protection dissolves around you, you're now exposed to the belly's self-cleaning cycle."
				T.Del()
				update_patient()
				return

			//Do digestion damage
			R << "DEBUG: Doing damage to [T.name]"
			T.adjustBruteLoss(3)
			T.adjustFireLoss(3)
			return
		...
		//Handle the target being anything but a /mob/living/carbon/human
			...
			var/obj/T = target
			R << "DEBUG: Deleting [T.name]"
			if(T in important_items)
				R << "DEBUG: Wait, no I'm not, it's important!"
				T.loc = R.loc
				playsound(loc, 'sound/effects/splat.ogg', 50, 1)
				R.visible_message("<span class='notice'>\The [T.name] spills out of [R.name]'s [eject_port].</span>", "<span class='notice'>\The [T.name] spills out of your [eject_port].</span>")
			for(var/obj/SubItem in T.contents) //Taking contents and putting them in belly
				SubItem.loc = src
				if(istype(SubItem,/mob/living)) //Friendly message for now-exposed mobs c:
					SubItem << "As your protection dissolves around you, you're now exposed to the belly's self-cleaning cycle."
					update_patient()
			T.Del()
			//R.cell.charge += 100 //Ace says no charge from digested items, OP
			return
	return
...
/obj/item/device/dogborg/sleeper/process()
	if(!src.occupied && !src.cleaning)
		...
		processing_objects.Remove(src) //Just in case???
		return
	if(src.cleaning)
		clean_cycle(src)
	if(patient.health < 0)
