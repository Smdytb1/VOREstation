...
/obj/item/weapon/dogborg/sleeper/K9 //The K9 portabrig
	...
	//Sleeper for non k9 or medihounds

/obj/item/weapon/dogborg/vore
	name = "Stomach Module"
	desc = "A stomach module installed in cyborgs to allow transport of crewmembers."
	icon = 'icons/mob/dogborg.dmi'
	icon_state = "sleeper"
	var/mob/living/carbon/patient = null
	var/mob/living/silicon/hound = null
	...
	var/min_health = -100
	var/occupied = 0
	var/list/injection_chems = list("digestive_enzymes") //They get to digest people

/obj/item/weapon/dogborg/vore/Exit(atom/movable/O)
	return 0

/obj/item/weapon/dogborg/vore/afterattack(mob/living/carbon/target, mob/living/silicon/user, proximity)
	if(!proximity)
		return
	if(!ishuman(target))
		return
	...
	if(!patient && ishuman(target) && !target.buckled && do_after (user, 50))
		...
		hound = user
		target.reset_view(src)
		user << "<span class='notice'>Your stomach lights up as [target] slips into your [src].</span>"
		user.visible_message("<span class='warning'>[user]'s stomach lights up as [target] slips inside into their [src].</span>")
		user.visible_message("[target] loaded.")
		src.occupied = 1
		var/mob/living/silicon/robot.R = user
		if(patient.stat < 2)
			R.sleeper_r = 0 //Basically, I'm leaving this in so it makes sure the sprite doesn't appear.
			R.sleeper_g = 0 //No matter what.
			R.update_icons()
		else
			R.sleeper_g = 0 //I /could/ remove these lines, but I don't want it bugging and causing the sprite to appear.
			R.sleeper_r = 0
			R.update_icons()
		processing_objects |= src

/obj/item/weapon/dogborg/vore/proc/go_out()
	if(src.occupied == 0)
		return
	var/mob/living/silicon/robot.R = hound
	hound << "<span class='notice'>[patient] ejected.</span>"
	R.sleeper_r = 0 //Same as above
	R.sleeper_g = 0 //Same as above
	R.update_icons()
	patient.forceMove(get_turf(src))
	patient.reset_view()
	patient = null
...
/obj/item/weapon/dogborg/vore/proc/drain()
	var/mob/living/silicon/robot.R = hound
	R.cell.charge = R.cell.charge - 5

/obj/item/weapon/dogborg/vore/attack_self(mob/user)
	if(..())
		return
	sleeperUI(user)

/obj/item/weapon/dogborg/vore/proc/sleeperUI(mob/user)
	var/dat
	dat += "<h3>Injector</h3>"
	if(patient)
		dat += "<A href='?src=\ref[src];inject=inaprovaline'>Inject Inaprovaline</A>"
	...
	if(!patient)
		...
		switch(patient.stat)
			if(0)
				...
				var/mob/living/silicon/robot.R = user
				R.sleeper_r = 0
				R.sleeper_g = 0 //So they don't get the custom sprite
				R.update_icons()
			if(1)
				dat += "<span class='average'>Unconscious</span>"
				var/mob/living/silicon/robot.R = user
				...
				var/mob/living/silicon/robot.R = user
				R.sleeper_g = 0
				R.sleeper_r = 0 //So they don't get the custom sprite.
				R.update_icons()
		dat += text("[]\t-Pulse, bpm: []</FONT><BR>", (patient.pulse == PULSE_NONE || patient.pulse == PULSE_THREADY ? "<font color='red'>" : "<font color='blue'>"), patient.get_pulse(GETPULSE_TOOL))
		dat += text("[]\t-Brute Damage %: []</FONT><BR>", (patient.getBruteLoss() < 60 ? "<font color='blue'>" : "<font color='red'>"), patient.getBruteLoss())
		dat += text("[]\t-Respiratory Damage %: []</FONT><BR>", (patient.getOxyLoss() < 60 ? "<font color='blue'>" : "<font color='red'>"), patient.getOxyLoss())
...
/obj/item/weapon/dogborg/vore/Topic(href, href_list)
	if(..() || usr == patient)
		return
	usr.set_machine(src)
	if(href_list["refresh"])
...
/obj/item/weapon/dogborg/vore/proc/inject_chem(mob/user, chem)
	if(patient && patient.reagents)
		if(chem in injection_chems + "inaprovaline")
			var/mob/living/silicon/robot.R = user
			if(R.cell.charge < 750) //This is so borgs don't kill theirselves with it.
...
/obj/item/weapon/dogborg/vore/process()
	if(src.occupied == 0)
		processing_objects.Remove(src)
		return
	if(patient.health > 0)
		patient.adjustOxyLoss(-1) //Heal some oxygen damage, as the borgs stomach is airtight. If the borg wants them dead, they can inject digestive enzymes.
		patient.updatehealth()
	patient.AdjustStunned(-4)
	patient.AdjustWeakened(-4)
	src.drain()
	if(patient.reagents.get_reagent_amount("inaprovaline") < 5) //This is so they don't suffocate to death in case anything bugs.
		patient.reagents.add_reagent("inaprovaline", 5) //I would remove this, but I don't want them suffocating. Digestive enzymes does enough damage to kill them outright, so it should be fine.
